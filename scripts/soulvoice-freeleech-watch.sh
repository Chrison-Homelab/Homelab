...
verdict = {"unique": len(d), "old": len(old), "old_free": len(old_free), "active": False}
if len(old) >= min_old and len(old_free) / len(old) >= min_ratio:
    verdict["active"] = True
...
print(f"unique={verdict['unique']} old={verdict['old']} old_free={verdict['old_free']} "
      f"active={verdict['active']} candidates={len(cand)}")
PY

ACTIVE=$(python3 -c "import json;print(json.load(open('$WORK/verdict.json'))['active'])")
if [ "$ACTIVE" != "True" ]; then
  echo "no site-wide freeleech detected; nothing to raise"
  exit 0
fi
