...
7. **CT lifecycle (create)** — `CommunityScriptsCreator` builds the same `mode=generated var_* … ct/<app>.sh` invocation as the BL-013 renderer (channel-aware, full var mapping) over SSH; `converge --apply` now **ensure-creates** each CT (skip if present) **then provisions**, so it can build a stack from zero. The forgejo-runner dev-script's create-time token requirement is satisfied by deriving `var_forgejo_*` create-vars only when the CT is absent. Built. ✅
8. **Member scoping — `--only`** (#306) — `converge <stack> --only <member>[,<member>…]` narrows **plan, apply and destroy** to named members. Filtering happens *after* load + topological ordering, so a selected member keeps its merged stack defaults and its position in the full dependency order; only the set of members acted on changes. Two guards: an unknown name is **fatal** (exit 2, nothing touched — silently converging nothing on a typo would report success), and a `dependsOn` left *outside* the selection is verified to exist live, failing that member rather than half-applying against a missing dependency. ✅

   **This retires the isolated-stack-directory workaround.** Podman phases 1 (#285) and 2a (#302) were applied from a hand-copied `stack.yaml` + one member, to avoid an apply that also wanted `SetConfig` on adopted VM 2000 (Home Assistant, describe-only) and a drift-fix on CT 4100. That workaround silently dropped `dependsOn` ordering and cross-member context. Use `--only` instead:

   ```bash
   dotnet run --project Infrastructure/engine -- converge stacks/SmartHome --only podman-host --apply
   ```

   Note this scopes the *symptom*. The root cause is that adopted members have no way to declare themselves unmanaged, so they resurface as perpetual drift in every plan; a shape-level `manage: describe-only` marker is the complementary fix and is still open.
9. **Remaining** (next): automate Forgejo first-run (its web wizard blocks a fully-unattended forgejo-runner from zero), resolve logical ingress hosts → CT IPs for live cloudflared create, ProxmoxSharp-native CT ops (start/stop/destroy beyond create), and a first authorized live `--apply`. ⏳
