...
8. **Remaining** (next): automate Forgejo first-run (its web wizard blocks a fully-unattended forgejo-runner from zero), resolve logical ingress hosts → CT IPs for live cloudflared create, ProxmoxSharp-native CT ops (start/stop/destroy beyond create), and a first authorized live `--apply`. ⏳
