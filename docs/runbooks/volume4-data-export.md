...
> **Update (2026-06-20):** SynoSharp is **no longer read-only** — the NFS export write
> path landed (SynoSharp PR #9, ADR-0002/#057). The Synology **export rule** is now
> created declaratively via `synosharp apply <spec>` (reproducible IaC; proven idempotent
> against the live NAS). The manual DSM steps below remain as reference / fallback. The
> Proxmox steps still run over SSH. Target node: **hpe-01** (where the stack + Plex live).
> ⚠ The live volume4 export is currently scoped to `10.10.0.0/16`; the Proxmox host mounts
> from `192.168.179.x`, so the export ACL must include the host's subnet too (task #1).
