...
# year -> bootstrapper channel (aka.ms/vs/<n>/release): 2019=16, 2022=17, 2026=18.
$editions = @(
  @{ Year = 2019; Channel = 16 },
  @{ Year = 2022; Channel = 17 },
  @{ Year = 2026; Channel = 18 }
)
...
function Invoke-VsInstall($edition, $product) {
  ...
  $url = "https://aka.ms/vs/$($edition.Channel)/release/$($product.Exe)"
...
try {
  ...
  if (Test-Path $vswhere) { & $vswhere -all -prerelease -property displayName }
}
finally {
